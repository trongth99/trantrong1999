...
export class AppService {
  ...
  processId = 'fis.onboarding.process.banking.mothetindung';
  username = 'customer';
  ...
  _completedChonTheTD(processInstanceId: any, data: any) {
    this.swalWarning('', 'Hệ thống đang xử lý ...', 60000);

    this._getTaskInstanceId(processInstanceId).subscribe(
      res => {
        let taskInstanceId = this._getTaskId(res, 'ChonTheTD');

        if (taskInstanceId && !this.taskInstanceIds[0].ChonTheTD) {
          this.completedTask(taskInstanceId, data).subscribe(
            res => {
              this.taskInstanceIds[0].ChonTheTD = taskInstanceId;
              Swal.close();
            },
            error => {
              this.errsStep = true;
              this.errsStepChonTheTD = true;
            }
          );
        } else if (this.taskInstanceIds[0].ChonTheTD) {
          this.updateTask(processInstanceId, 'loaiTheTDung', data['loaiTheTDung']['fis.onboarding.process.banking.model.LoaiTheTDung']).subscribe(
            res => {
              Swal.close();
            },
            error => {
    ...
    this._getTaskInstanceId(processInstanceId).subscribe(
      res => {
        ...
        if (taskInstanceId && !this.taskInstanceIds[0].NhapDonDKy) {
          ...
          let arrayNhapDonDangKy = ['ttinCNhan', 'ttBMat', 'ttVLam', 'theTDung'];
          let array = ['TTinCNhan', 'TTinBMat', 'TTinVLam', 'TheTDung'];
