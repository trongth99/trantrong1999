...
export class AppComponent {
  ...
  configSteps = [
    ...
    {
      status: 1,
      title: 'Đơn đăng ký mở khoản vay'
    },
    {
      status: 1,
      title: 'Đơn đăng ký mở khoản vay'
  ...
  theSPVay = [
    {maSPVay: "vtieudung", tenSPVay: "Vay tiêu dùng"},
    {maSPVay: "voto", tenSPVay: "Vay mua ô tô"},
    {maSPVay: "vbds", tenSPVay: "Vay mua bất động sản"},
    {maSPVay: "vkd", tenSPVay: "Vay sản xuất kinh doanh"},
  ...
  fileHKTT = '';
  fileNameHKTT = '';
  lastModifileHKTT: any;
  iconDeleteHKTT = false;

  fileGDKKH = '';
  fileNameGDKKH = '';
  lastModifileGDKKH: any;
  iconDeleteGDKKH = false;
  ...
  fileMBCH = '';
  fileNameMBCH = '';
  lastModifileMBCH: any;
  iconDeleteMBCH = false;
  ...
  loadForm() {
    ...
    this._17FormGroup = this._formBuilder.group({});
  }
  ...
  step7Handle() {
    ...
    let data = {
      "spVay": {
        "fis.onboarding.process.banking.model.SPVay": {
          "maSPVay": this.maSPVay,
          "tenSPVay": this.tenSPVay,
          "laiSPVay": ""
        }
      }

    };
    this.appService._completedChonSPVay(this.processInstanceId, data);
...
/////
    ...
    let data = {
      ...
      "ttinNCauVay": {
        "fis.onboarding.process.banking.model.TTinNCauVay": {
          "phuongThucVay": "",
          "soTienVay": "",
          "thoiHanVay": "",
          "mucDichVay": "",
  ...
  cValid(index: number) {
    ...
    switch (step) {
      ...
      case 16:
        return this._16FormGroup.valid;
      case 17:
        return this._17FormGroup.valid;
      default:
        return false;
    }
  }
  ...
  onChangeLoaiTheTinDung(e: any) {
    ...
    if (vals) {
      this.maSPVay = vals.split('|')[0];
      this.tenSPVay = vals.split('|')[1];
    }
  }
  ...
  checkCMMDV() {
    this.isShowCMMDV = !this.isShowCMMDV;
  }

  checkTSDB() {
    this.isShowTSDB = !this.isShowTSDB;
  }
  ...
  deleteFile(type: string) {
    if (type == 'file_tt') {
      this.fileTT = '';
      this.fileNameTT = '';
      this.lastModifileTT = 0;
      this.iconDeleteTT = !this.iconDeleteTT;
    } else if (type == 'file_gcn') {
      this.fileGCN = '';
      this.fileNameGCN = '';
      this.lastModifileGCN = 0;
      this.iconDeleteGCN = !this.iconDeleteGCN;
    } else if (type == 'file_gt') {
      this.fileGT = '';
      this.fileNameGT = '';
      this.lastModifileGT = 0;
      this.iconDeleteGT = !this.iconDeleteGT;
    } else if (type == 'file_moo') {
      this.fileMOO = '';
      this.fileNameMOO = '';
      this.lastModifileMOO = 0;
      this.iconDeleteMOO = !this.iconDeleteMOO;
    } else if (type == 'file_hdx') {
      this.fileHDX = '';
      this.fileNameHDX = '';
      this.lastModifileHDX = 0;
      this.iconDeleteHDX = !this.iconDeleteHDX;
    } else if (type == 'file_mbch') {
      this.fileMBCH = '';
      this.fileNameMBCH = '';
      this.lastModifileMBCH = 0;
      this.iconDeleteMBCH = !this.iconDeleteMBCH;
    } else if (type == 'file_sdv') {
      this.fileSDV = '';
      this.fileNameSDV = '';
      this.lastModifileSDV = 0;
      this.iconDeleteSDV = !this.iconDeleteSDV;
    } else if (type == 'file_gdkkh') {
      this.fileGDKKH = '';
      this.fileNameGDKKH = '';
      this.lastModifileGDKKH = 0;
      this.iconDeleteGDKKH = !this.iconDeleteGDKKH;
    } else if (type == 'file_hktt') {
      this.fileHKTT = '';
      this.fileNameHKTT = '';
      this.lastModifileHKTT = 0;
      this.iconDeleteHKTT = !this.iconDeleteHKTT;
    ...
    if (file) {
      ...
      reader.onload = () => {
        ...
        if (typeof content !== "undefined") {
          ...
          if (type == 'file_tt') {
            this.fileTT = this.contentBase64;
            this.fileNameTT = file.name;
            this.lastModifileTT = file.lastModified;
            this.iconDeleteTT = !this.iconDeleteTT;
          } else if (type == 'file_gcn') {
            this.fileGCN = this.contentBase64;
            this.fileNameGCN = file.name;
            this.lastModifileGCN = file.lastModified;
            this.iconDeleteGCN = !this.iconDeleteGCN;
          } else if (type == 'file_gt') {
            this.fileGT = this.contentBase64;
            this.fileNameGT = file.name;
            this.lastModifileGT = file.lastModified;
            this.iconDeleteGT = !this.iconDeleteGT;
          } else if (type == 'file_moo') {
            this.fileMOO = this.contentBase64;
            this.fileNameMOO = file.name;
            this.lastModifileMOO = file.lastModified;
            this.iconDeleteMOO = !this.iconDeleteMOO;
          } else if (type == 'file_hdx') {
            this.fileHDX = this.contentBase64;
            this.fileNameHDX = file.name;
            this.lastModifileHDX = file.lastModified;
            this.iconDeleteHDX = !this.iconDeleteHDX;
          } else if (type == 'file_mbch') {
            this.fileMBCH = this.contentBase64;
            this.fileNameMBCH = file.name;
            this.lastModifileMBCH = file.lastModified;
            this.iconDeleteMBCH = !this.iconDeleteMBCH;
          } else if (type == 'file_sdv') {
            this.fileSDV = this.contentBase64;
            this.fileNameSDV = file.name;
            this.lastModifileSDV = file.lastModified;
            this.iconDeleteSDV = !this.iconDeleteSDV;
          } else if (type == 'file_gdkkh') {
            this.fileGDKKH = this.contentBase64;
            this.fileNameGDKKH = file.name;
            this.lastModifileGDKKH = file.lastModified;
            this.iconDeleteGDKKH = !this.iconDeleteGDKKH;
          } else if (type == 'file_hktt') {
            this.fileHKTT = this.contentBase64;
            this.fileNameHKTT = file.name;
            this.lastModifileHKTT = file.lastModified;
            this.iconDeleteHKTT = !this.iconDeleteHKTT;
          } else if (type == 'file_hdld') {
